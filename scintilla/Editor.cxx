...
SelectionPosition Editor::SPositionFromLocation(Point pt, bool canReturnInvalid, bool charPosition, bool virtualSpace) {
	...
	if (drawSurface && ll) {
		...
		if (subLine < ll->lines) {
			...
			float subLineStart = ll->positions[lineStart];
			...
			if (virtualSpace) {
				const float spaceWidth = vs.styles[ll->EndLineStyle()].spaceWidth;
		...
		if (wrapState == eWrapNone) {
			...
			if (drawSurface) {
				...
				while (lineToWrap < lastLineToWrap) {
					if (WrapOneLine(drawSurface, lineToWrap)) {
						wrapOccurred = true;
					}
					lineToWrap++;
				}
...
void Editor::LinesSplit(int pixelWidth) {
	if (!RangeContainsProtected(targetStart, targetEnd)) {
		...
		for (int line = lineStart; line <= lineEnd; line++) {
			...
			if (drawSurface && ll) {
				unsigned int posLineStart = pdoc->LineStart(line);
				LayoutLine(line, drawSurface, vs, ll, pixelWidth);
		...
		if (vs.ms[margin].width > 0) {
			...
			while ((visibleLine < cs.LinesDisplayed()) && yposScreen < rcMargin.bottom) {
				...
				if (vs.ms[margin].style == SC_MARGIN_NUMBER) {
					...
					// Right justify
					float width = surface->WidthText(vs.styles[STYLE_LINENUMBER].font, number, istrlen(number));
					float xpos = rcNumber.right - width - 3;
					rcNumber.left = xpos;
...
void Editor::LayoutLine(int line, Surface *surface, ViewStyle &vstyle, LineLayout *ll, int width) {
	...
	if (ll->validity == LineLayout::llInvalid) {
		...
		ll->positions[0] = 0;
		float tabWidth = vstyle.spaceWidth * pdoc->tabInChars;
		bool lastSegItalics = false;
		Font &ctrlCharsFont = vstyle.styles[STYLE_CONTROLCHAR].font;

		float ctrlCharWidth[32] = {0};
	...
	if ((ll->validity == LineLayout::llPositions) || (ll->widthLine != width)) {
		...
		if (width == LineLayout::wrapWidthInfinite) {
			...
			float wrapAddIndent = 0; // This will be added to initial indent of line
...
Colour Editor::SelectionBackground(ViewStyle &vsDraw, bool main) {
	return main ?
		(primarySelection ? vsDraw.selbackground : vsDraw.selbackground2) :
		vsDraw.selAdditionalBackground;
}

Colour Editor::TextBackground(ViewStyle &vsDraw, bool overrideBackground,
        Colour background, int inSelection, bool inHotspot, int styleMain, int i, LineLayout *ll) {
	if (inSelection == 1) {
...
void Editor::DrawWrapMarker(Surface *surface, PRectangle rcPlace,
        bool isEndMarker, Colour wrapColour) {
...
static void SimpleAlphaRectangle(Surface *surface, PRectangle rc, Colour fill, int alpha) {
...
void DrawTextBlob(Surface *surface, ViewStyle &vsDraw, PRectangle rcSegment,
				  const char *s, Colour textBack, Colour textFore, bool twoPhaseDraw) {
	if (!twoPhaseDraw) {
...
void Editor::DrawEOL(Surface *surface, ViewStyle &vsDraw, PRectangle rcLine, LineLayout *ll,
        ...
        bool drawWrapMarkEnd, Colour wrapColour) {
	...
	float virtualSpace = 0;
	if (lastSubLine) {
		const float spaceWidth = vsDraw.styles[ll->EndLineStyle()].spaceWidth;
		virtualSpace = sel.VirtualSpaceFor(pdoc->LineEnd(line)) * spaceWidth;
	}
	float xEol = ll->positions[lineEnd] - subLineStart;
	...
	if (lastSubLine) {
		for (int eolPos=ll->numCharsBeforeEOL; eolPos<ll->numCharsInLine; eolPos++) {
			...
			Colour textBack = TextBackground(vsDraw, overrideBackground, background, eolInSelection, false, styleMain, eolPos, ll);
			Colour textFore = vsDraw.styles[styleMain].fore;
...
void Editor::DrawIndicator(int indicNum, int startPos, int endPos, Surface *surface, ViewStyle &vsDraw,
	...
	const float subLineStart = ll->positions[ll->LineStart(subLine)];
	PRectangle rcIndic(
...
void Editor::DrawLine(Surface *surface, ViewStyle &vsDraw, int line, int lineVisible, int xStart,
	...
	Colour background = MakeRGBA(0, 0, 0, 255);
	...
	const float indentWidth = pdoc->IndentSize() * vsDraw.spaceWidth;
	const float epsilon = 0.0001f;	// A small nudge to avoid floating point precision issues

	int posLineStart = pdoc->LineStart(line);

	int startseg = ll->LineStart(subLine);
	double subLineStart = ll->positions[startseg];
	if (subLine >= ll->lines) {
	...
	Colour wrapColour = vsDraw.styles[STYLE_DEFAULT].fore;
	...
	while (twoPhaseDraw && (next < lineEnd)) {
		...
		if ((rcSegment.left <= rcLine.right) && (rcSegment.right >= rcLine.left)) {
			...
			Colour textBack = TextBackground(vsDraw, overrideBackground, background, inSelection, inHotspot, styleMain, i, ll);
			if (ll->chars[i] == '\t') {
				// Tab display
						...
						if (ll->chars[cpos + startseg] == ' ') {
							if (vsDraw.viewWhitespace != wsInvisible) {
								if (vsDraw.whitespaceForegroundSet)
									textFore = vsDraw.whitespaceForeground;
								if (!inIndentation || vsDraw.viewWhitespace == wsVisibleAlways) {
									float xmid = (ll->positions[cpos + startseg] + ll->positions[cpos + startseg + 1]) / 2;
...
void Editor::DrawBlockCaret(Surface *surface, ViewStyle &vsDraw, LineLayout *ll, int subLine,
	...
	if ((ll->wrapIndent != 0) && (lineStart != 0)) {
		float wordWrapCharWidth = ll->wrapIndent;
...
void Editor::RefreshPixMaps(Surface* /*surfaceWindow*/) {
	...
	if (!IsPixmapInitialised(pixmapSelPattern)) {
		...
		Colour colourFMFill = vs.selbar;
		Colour colourFMStripes = vs.selbarlight;

		if (!(vs.selbarlight == MakeRGBA(0xff, 0xff, 0xff))) {
...
void Editor::DrawCarets(Surface *surface, ViewStyle &vsDraw, int lineDoc, int xStart,
	...
	for (size_t r=0; (r<sel.Count()) || drawDrag; r++) {
		...
		const float spaceWidth = vsDraw.styles[ll->EndLineStyle()].spaceWidth;
		const float virtualOffset = posCaret.VirtualSpace() * spaceWidth;
		if (ll->InLine(offset, subLine) && offset <= ll->numCharsBeforeEOL) {
			...
			if ((xposCaret >= 0) && (vsDraw.caretWidth > 0) && (vsDraw.caretStyle != CARETSTYLE_INVISIBLE) &&
				...
				Colour caretColour = mainCaret ? vsDraw.caretcolour : vsDraw.additionalCaretColour;
				if (drawBlockCaret) {
...
void Editor::Paint(/*Surface *surfaceWindow, PRectangle rcArea*/) {
	...
	if (paintState != paintAbandoned) {
		...
		if (rcArea.Intersects(rcRightMargin)) {
			drawSurface->FillRectangle(rcRightMargin, vs.styles[STYLE_DEFAULT].back);
		}
	}
	...
	if (rcArea.right > vs.fixedColumnWidth) {
		...
		drawSurface->SetClip(rcTextArea);
		...
		if (rcBeyondEOF.top < rcBeyondEOF.bottom) {
			drawSurface->FillRectangle(rcBeyondEOF, vs.styles[STYLE_DEFAULT].back);
			if (vs.edgeState == EDGE_LINE) {
				int edgeX = theEdge * vs.spaceWidth;
				rcBeyondEOF.left = edgeX + xStart;
				rcBeyondEOF.right = rcBeyondEOF.left + 1;
				drawSurface->FillRectangle(rcBeyondEOF, vs.edgecolour);
			}
		}
...
Colour InvertedLight(Colour orig) {
	unsigned int r = GetRed(orig);
	unsigned int g = GetGreen(orig);
	unsigned int b = GetBlue(orig);
	...
	g = g * il / l;
	b = b * il / l;
	return MakeRGBA(Platform::Minimum(r, 0xff), Platform::Minimum(g, 0xff), Platform::Minimum(b, 0xff));
}
...
long Editor::FormatRange(bool draw, Sci_RangeToFormat *pfr) {
	...
	if (lineNumberIndex >= 0) {
		lineNumberWidth = drawSurface->WidthText(vsPrint.styles[STYLE_LINENUMBER].font,
	...
	while (lineDoc <= linePrintLast && ypos < pfr->rc.bottom) {
		...
		// using each surface.
		drawSurface->FlushCachedState();

		// Copy this line and its styles from the document into local arrays
		// and determine the x position at which each character starts.
		LineLayout ll(8000);
		LayoutLine(lineDoc, drawSurface, vsPrint, &ll, widthPrint);
		...
		if (draw && lineNumberWidth &&
			...
			// Right justify
			rcNumber.left = rcNumber.right - drawSurface->WidthText(
			            vsPrint.styles[STYLE_LINENUMBER].font, number, istrlen(number));
			drawSurface->FlushCachedState();
			drawSurface->DrawTextNoClip(rcNumber, vsPrint.styles[STYLE_LINENUMBER].font,
			...
			if (ypos + vsPrint.lineHeight <= pfr->rc.bottom) {
				if (visibleLine >= 0) {
					if (draw) {
						rcLine.top = ypos;
						rcLine.bottom = ypos + vsPrint.lineHeight;
						DrawLine(drawSurface, vsPrint, lineDoc, visibleLine, xStart, rcLine, &ll, iwl);
					}
...
void Editor::CopySelectionRange(SelectionText *ss, bool allowLineCopy) {
	if (sel.Empty()) {
		if (allowLineCopy) {
			...
			ss->Set(textWithEndl, static_cast<int>(strlen(textWithEndl) + 1), false, true);
			delete []text;
		}
	} else {
		...
		text[size] = '\0';
		ss->Set(text, static_cast<int>(size + 1), sel.IsRectangular(), sel.selType == Selection::selLines);
	}
}
...
sptr_t Editor::Command(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_MARKERSETBACKSELECTED:
		if (wParam <= MARKER_MAX)
			vs.markers[wParam].backSelected = Colour(lParam);
		InvalidateStyleData();
		break;
